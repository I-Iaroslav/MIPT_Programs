...
#define TIME 4000 //êîëè÷åñòâî èòåðàöèé

//ôóíêöèÿ äëÿ êðàñèâîãî âûâîäà âðåìåíè)
...
void print_map(bool ** map, int size_x, int size_y, HANDLE &hConsole, DWORD &dwBytesWritten) {
	wchar_t* screen = new wchar_t[2 * size_x * size_y + 1]; // Ìàññèâ äëÿ çàïèñè â áóôåð
	...
	screen[2 * size_x * size_y] = '\0';  // Ïîñëåäíèé ñèìâîë - îêîí÷àíèå ñòðîêè
	WriteConsoleOutputCharacter(hConsole, screen, 2 * size_x * size_y, { 0, 0 }, &dwBytesWritten); // Çàïèñü â áóôåð
...
int main(int argc, char* argv[]) {
	...
	SetConsoleActiveScreenBuffer(hConsole); // Íàñòðîéêà êîíñîëè
	...
	//îïðåäåëÿåì, êàêèì ïðîöåññàì äîñòàíåòñÿ áîëüøå ðàáîòû
	...
	if (ProcRank != 0) {
		...
		MPI_Send(&column_per_process, sizeof(column_per_process), MPI_BYTE, 0, 1, MPI_COMM_WORLD); //ó ïðîöåññîâ ìá ðàçíîå êîëëè÷åñòâî îáðàáàòûâàåìûõ ñòîëáöîâ, ïîýòîìó ïåðåäàåì èõ êîëè÷åñòâî
		...
		for (int i = 1; i < ProcNum; ++i) {
			//ïðèíèìàåì ó êàæäîãî ïðîöåññà äàííûå
			int rec_column_PerProcess;//êîëëè÷åñòâî îáðàáàòûâàåìûõ ñòîëáöîâ ó ïðîöåññà, êîòîðûé íàì ïåðåñûëàåò äàííûå
