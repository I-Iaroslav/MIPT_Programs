...
#define PAYLOAD_SIZE 1400       //Максимальная длинна данных в пакете
#define PACKET_HEADER_SIZE 24   //Размер заголовка

#pragma pack(push, 1)
struct Packet
{
    uint64_t id;                //Уникальный id изображения
    uint64_t size;              //Размер изображения в байтах
    uint32_t part_id;           //Номер части
    uint32_t part_count;        //Количиство частей
    uint8_t data[PAYLOAD_SIZE]; //Данные
};
#pragma pack(pop)
...
void send_to_udp(std::vector<uint8_t> image_data, std::vector<uint8_t> sha512sum, std::string address, int port)
{
    ...
    std::shuffle(partindex.begin(), partindex.end(), rng);                      //Переставляет элементы в случайном порядке
...
int main(int argc, char** argv)
{
    ...
    int port = std::stoi(argv[3]);                                                                                  //Преобразование строки в целое число
    
    std::ifstream image(filename, std::ios::binary);                                                                //Открываем файл в биинарном виде
    std::vector<uint8_t> image_data((std::istreambuf_iterator<char>(image)), std::istreambuf_iterator<char>());     //Представляем файл как вектор символов
    std::vector<uint8_t> sha512sum(SHA512_DIGEST_LENGTH);
    SHA512(image_data.data(), image_data.size(), sha512sum.data());                                                 //Вычисляем хэш
